    ...
    for (let type in addons) {
      ...
      for (let addon in addons[type]) {
        ...
        if (addons[type][addon].price === 0 || addons[type][addon].purchased === true) {
          button =
          `<br><button class="toggle-install button blue small" data-plugin="${addons[type][addon].ID}" class="install-plugin-btn"><span>Install Plugin</span><i class="icon-spin5 animate-spin"></i></button><a class="read-more-addon" href="http://formcraft-wp.com/addons/?page_id=${addons[type][addon].ID}" target="_blank">read more</a>`
        } else {
